...
#Objective 2 --> Outlier Detection in Total Collection
plt.figure(figsize=(5, 7))
sns.boxplot(y=data['Total Collection'], color='teal')
plt.title("Outlier Detection: Total Collection")
plt.ylabel("Total Collection")
plt.tight_layout()
plt.show()

#Objective 3 --> Trend of Library Establishment Over the Years
...
#Objective 4 --> KDE Plot of Circulation Per Capita
plt.figure(figsize=(8, 5))
sns.kdeplot(data['Circulation Per Capita Served'], fill=True, color='purple')
plt.title("KDE Plot: Circulation Per Capita Served")
plt.xlabel("Circulation Per Capita Served")
plt.tight_layout()
plt.show()

#Objective 5 -->Histogram of Reference Questions
plt.figure(figsize=(8, 5))
plt.hist(data['Reference Questions'], bins=30, color='red', edgecolor='black')
...
#Objective 6--> Correlation Between Numeric Features
...
#Objective 7 --> Distribution of Library Types
library_types = data['Principal Public?'].value_counts()
plt.figure(figsize=(6, 6))
plt.pie(library_types, labels=library_types.index, autopct='%1.1f%%', colors=['#00ff00','#ff0000'])
...
sns.scatterplot(data=data, x='Total Collection', y='Total Library Visits', color='darkgreen')
...
#Objective 9 --> Line Plot of Program Attendance by Population Range
...
attendance_by_pop.plot(kind='line', marker='o', color='blue')
...
data.to_csv('new_cleaned_file.csv')
